<?php
    include "db_config.php";
        class user
        {
            ...
            {
                // FIXED: Using DB_DATABASE constant instead of hardcoded 'hotel'
            ...
            public function reg_user($name, $username, $password, $email)
            {
                ...
                if($count_row==0)
                {
                    // FIXED: Properly hash the password
                    $hashed_password = password_hash($password, PASSWORD_DEFAULT);
                    $sql1="INSERT INTO manager SET uname='$username', upass='$hashed_password', fullname='$name', uemail='$email'";
...
{
    $sql2="SELECT uid, upass from manager WHERE (uemail='$emailusername' OR uname='$emailusername')";
    ...
    if($count_row==1)
    {
        $db_password = $user_data['upass'];
        
        // Check if password is hashed (starts with $2y$ and is 60 chars)
        $is_hashed = (strlen($db_password) == 60 && substr($db_password, 0, 4) == '$2y$');
        ...
        if($password_match)
        {
            // If it was plain text, update it to hashed
            if(!$is_hashed) {
                $new_hash = password_hash($password, PASSWORD_DEFAULT);
                $update_sql = "UPDATE manager SET upass='$new_hash' WHERE uid=" . $user_data['uid'];
                mysqli_query($this->db, $update_sql);
            }
